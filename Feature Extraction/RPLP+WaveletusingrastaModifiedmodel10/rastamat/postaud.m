...
% Include frequency points at extremes, discard later
 nfpts = nbands+2*broaden; %commented for rplp 
% nfpts= 257;
fmin=300;
...
%   bandcfhz = mel2hz(linspace(0, hz2mel(fmax), nfpts));
    bandcfhz = mel2hz(linspace(hz2mel(fmin), hz2mel(fmax), nfpts));
...
% fsq = bandcfhz.^2;                                        %commented for rplp
% ftmp = fsq + 1.6e5;                                       %commented for rplp
% eql = ((fsq./ftmp).^2) .* ((fsq + 1.44e6)./(fsq + 9.61e6));%commented for rplp
eql=bandcfhz; %added for rplp
% weight the critical bands

s=repmat(eql',1,nframes);
...
if (broaden)  % By default, broaden=0
